...
parents = np.array([-1, 0, 1, 2, 3, 0, 5, 6, 7, 8, 9, 10, 11, 8, 13, 14, 8, 16, 17, 18, 0, 20, 21, 22])
...
def animation_plot(animations, fps=60):
    ...
    for ai in range(len(animations)):
        ...
        if len(anim) == 7:
            pos, rot, vel, ang, contact, foot_indicies, Xparents = \
                anim[0], anim[1], anim[2], anim[3], anim[4], anim[5], anim[6]
            Grot, Gpos, Gvel, Gang = quat.fk_vel(rot, pos, vel, ang, Xparents)
        else:
            pos, rot, contact, foot_indicies, Xparents = \
                anim[0], anim[1], anim[2], anim[3], anim[4]
            Grot, Gpos = quat.fk(rot, pos, Xparents)

        # TODO: 여기 heuristic 하게 scale 조정 해야함
...
if __name__ == '__main__':
    ...
    bvh_file = '../bvh/Jump_Angry_Ogre_001.bvh'
